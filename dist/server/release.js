...
async function run() {
    ...
    try {
        const sqlDir = (0, path_1.join)(__dirname, 'sql');
        const schemaPath = (0, path_1.join)(sqlDir, 'schema.sql');
        const seedPath = (0, path_1.join)(sqlDir, 'seed.sql');
        if (!(0, fs_1.existsSync)(schemaPath) || !(0, fs_1.existsSync)(seedPath)) {
            console.log('SQL files not found in dist; skipping DB setup.');
            return;
        }
        const schema = (0, fs_1.readFileSync)(schemaPath, 'utf8');
        const seed = (0, fs_1.readFileSync)(seedPath, 'utf8');
