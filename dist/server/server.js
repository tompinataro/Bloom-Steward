...
exports.app.post('/api/visit-state/reset', requireAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ ok: false, error: 'unauthorized' });
...
exports.app.post('/api/admin/users', requireAuth, requireAdmin, async (req, res) => {
    ...
    try {
        const result = await (0, db_1.dbQuery)(`insert into users (name, email, password_hash, role, must_change_password, managed_password, phone)
       values ($1, $2, $3, $4, true, $5, $6)
       returning id, name, email, coalesce(role, 'tech') as role, must_change_password, managed_password, phone`, [trimmedName, normalizedEmail, passwordHash, normalizedRole, tempPassword, phone]);
...
exports.app.post('/api/admin/users/:id/password', requireAuth, requireAdmin, async (req, res) => {
    if (!ensureDatabase(res))
        return;
    ...
    try {
        const hash = encryptLib.encryptPassword(newPassword);
        await (0, db_1.dbQuery)('update users set password_hash = $1, managed_password = $2, must_change_password = false where id = $3', [hash, newPassword, userId]);
        res.json({ ok: true });
    }
    catch (err) {
...
exports.app.post('/api/admin/routes/clear-for-tech', requireAuth, requireAdmin, async (req, res) => {
    if (!ensureDatabase(res))
        return;
    ...
    try {
        const result = await (0, db_1.dbQuery)(`delete from routes_today where user_id = $1`, [userId]);
...
exports.app.post('/api/admin/service-routes/:id/tech', requireAuth, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const clientRows = await (0, db_1.dbQuery)(`select
         c.id as client_id,
       ...
       where c.service_route_id = $1`, [routeId]);
        ...
        if (clientIds.length > 0) {
            if (userId === null) {
                ...
                await (0, db_1.dbQuery)('delete from routes_today where client_id = any($1::int[])', [clientIds]);
            }
            else {
           ...
           on conflict (client_id) do update set user_id = excluded.user_id, scheduled_time = excluded.scheduled_time`, [userId, routeId]);
            }
        }
        res.json({ ok: true });
    }
