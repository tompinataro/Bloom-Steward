...
function resolveRange(frequency, explicitStart, explicitEnd) {
    const parsedStart = explicitStart ? parseDateInZone(explicitStart, config_1.REPORT_TIMEZONE) : null;
    const parsedEnd = explicitEnd ? parseDateInZone(explicitEnd, config_1.REPORT_TIMEZONE) : null;
    ...
    if (!parsedStart || !parsedEnd) {
        switch (frequency) {
            case 'daily':
                start = startOfDayInZone(now, config_1.REPORT_TIMEZONE);
                end = endOfDayInZone(now, config_1.REPORT_TIMEZONE);
                break;
            case 'weekly': {
                const startAnchor = addDaysInZone(now, -7, config_1.REPORT_TIMEZONE);
                start = startOfDayInZone(startAnchor, config_1.REPORT_TIMEZONE);
                end = endOfDayInZone(now, config_1.REPORT_TIMEZONE);
                break;
            }
            case 'payperiod': {
                const startAnchor = addDaysInZone(now, -14, config_1.REPORT_TIMEZONE);
                start = startOfDayInZone(startAnchor, config_1.REPORT_TIMEZONE);
                end = endOfDayInZone(now, config_1.REPORT_TIMEZONE);
                break;
            }
            case 'monthly': {
                const startAnchor = addDaysInZone(now, -30, config_1.REPORT_TIMEZONE);
                start = startOfDayInZone(startAnchor, config_1.REPORT_TIMEZONE);
                end = endOfDayInZone(now, config_1.REPORT_TIMEZONE);
                break;
            }
