...
if (config_1.SMTP_URL) {
    mailTransport = nodemailer_1.default.createTransport(config_1.SMTP_URL);
}
else if (config_1.SMTP_HOST && config_1.SMTP_USER && config_1.SMTP_PASS) {
    mailTransport = nodemailer_1.default.createTransport({
        host: config_1.SMTP_HOST,
        port: config_1.SMTP_PORT || 587,
...
async function buildSummary(startDate, endDate) {
    ...
    const uniqueTechIds = Array.from(new Set(completedRows.map(r => r.tech_id).filter(Boolean)));
    ...
    for (const row of completedRows) {
        ...
        try {
            if (row.visit_time) {
                const d = new Date(row.visit_time);
                if (!Number.isNaN(d.getTime()))
...
function buildHtml(rows, start, end) {
  ...
  `;
    ...
    <p>Range: ${(0, range_1.formatRangeLabel)(start, end)}</p>
...
async function sendReportEmail(to, subject, html, csv) {
    ...
    await mailTransport.sendMail({
        from: config_1.SMTP_USER || undefined,
        to,
        subject,
        html,
