...
```bash
# Get admin token
TOKEN=$(curl -s -X POST https://bloom-steward-2a872c497756.herokuapp.com/api/auth/login \
  -H 'Content-Type: application/json' \
  -d '{"email":"marc@bloomsteward.com","password":"83472618"}' | grep -o '"token":"[^"]*' | cut -d'"' -f4)

# Run seed
curl -X POST https://bloom-steward-2a872c497756.herokuapp.com/api/admin/run-seed \
...
### Admin Account
- marc@bloomsteward.com / 83472618 → Can access /api/admin/* endpoints
